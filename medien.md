...
| Delete | All directories, sub-directories  and medias will be deleted. Allocations to pages and data sets will be completely deleted |
...
Please note: Do not upload media files using cmyk format. Furthermore you should take care that the media files resolution is not that high (do not use it directly from the camera). In some cases you will get a problem (depends on server configurations). The file size should not exceed the set upload values (Via *CMS settings -> Upload Configuration*).