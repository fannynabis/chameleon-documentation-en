...
| Create | Gives you a list with every available and existing module. It depends on the configuration of the slots which are set up by the administrator if the list differs according to the slots. So for example if you have a narrow gap in your layout with module slots in it, you can adjust them in such a way that only modules are selectable, which would match in it. |
